...
wt.changelog = {
    {
        ...
        "#F_熱修復（版本 3.0.1）：_#",
        "在多個位置加入了保護機制，以避免遺失或無效的資源檔案路徑（字體或材質）導致嚴重錯誤。",
        "#N_新增:_#",
        "新增對 Midnight 12.0.5 的支援。",
        "先前加入的設定右鍵選單進一步強化，新增複製與貼上功能，可輕鬆在相似類型的設定之間移動數值。",
        "新增一個用於管理字體選項的進階設定範本（未來更新將加入更多字體自訂功能）。",
        "#C_變更:_#",
        "設定中的數字滑桿外觀已更新，以符合新的暴雪滑桿，同時保留所有基於 Widget Tools Toolboxes 的插件強化功能。",
        "Toolbox 的載入結構已全面重製，不再支援舊版本。",
        "許多基本工具函式已移交給 Widget Tools（不再是 Toolbox 專屬），可透過 WidgetTools.utilities 全域存取。",
        "Toolbox 專屬資料將不再注入至框架表，而是存放於 Toolbox 專屬表中（包含提示訊息或容器內容排列資料）。",
        "管理暴雪全域 OnEvent（以及自訂事件）處理器的事件後端系統已更新，並新增可透過 WidgetTools.utilities 全域存取的工具函式。",
        "可自訂的框架、按鈕與其他元件現在必須透過新的建構子建立，其基礎版本中的自訂旗標已被移除。",
        "大部分僅供開發使用的註解已移出插件安裝檔案，以大幅減少安裝大小。",
        "資料管理設定頁面（現稱為「設定檔頁面」）的建構邏輯已拆分為 profilemanager 元件與其上的 GUI 變體，以提供更高的開發彈性與自訂能力。",
        "其他多項底層改善與優化。",
        "#F_熱修復項:_#",
        "許多其他小型修正。",
