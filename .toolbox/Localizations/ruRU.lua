...
wt.changelog = {
    {
        ...
        "В нескольких местах были добавлены защитные механизмы, предотвращающие критические ошибки, вызванные отсутствующими или некорректными путями к файлам ресурсов (шрифты или текстуры).",
        "#N_Новое:_#",
        "Добавлена поддержка Midnight 12.0.5.",
        "Ранее добавленные контекстные меню (ПКМ) для настроек были улучшены и теперь поддерживают копирование и вставку, что позволяет легко переносить значения между похожими типами настроек.",
        "Добавлен новый шаблон расширенных настроек для управления параметрами шрифтов (в будущих обновлениях появится больше возможностей кастомизации).",
        "#C_Изменения:_#",
        "Внешний вид числовых ползунков настроек обновлён в соответствии с новыми ползунками Blizzard, при этом все улучшенные функции для аддонов, созданных с помощью Widget Tools Toolboxes, сохранены.",
        "Структура загрузки Toolbox была переработана; старые версии больше не поддерживаются.",
        "Многие базовые утилитарные функции были переданы в Widget Tools (и больше не являются специфичными для Toolbox), доступные глобально через коллекцию WidgetTools.utilities.",
        "Данные, специфичные для Toolbox, больше не будут внедряться в таблицы фреймов, а будут храниться в отдельных таблицах Toolbox (включая данные о размещении подсказок или содержимого контейнеров).",
        "Обновлена backend‑система обработки событий, управляющая глобальными обработчиками Blizzard OnEvent (и пользовательскими событиями) для фреймов, с новыми утилитами, доступными глобально через WidgetTools.utilities.",
        "Настраиваемые фреймы, кнопки и другие виджеты теперь должны создаваться через новые конструкторы; флаги кастомизации удалены из их базовых версий.",
        ...
        "Логика построения страницы управления данными (теперь называемой страницей профилей) была разделена на виджет profilemanager и накладываемую GUI‑мутацию, что обеспечивает большую гибкость и возможности кастомизации.",
        "Несколько других внутренних изменений и улучшений.",
        "#F_Хотфиксы:_#",
