...
wt.changelog = {
    {
        ...
        "#F_热修复（版本 3.0.1）：_#",
        "在多个位置添加了保护措施，以防止缺失或无效的资源文件路径（字体或材质）导致严重错误。",
        "#N_新增:_#",
        "新增对 Midnight 12.0.5 的支持。",
        "此前添加的设置右键菜单进一步增强，加入了复制与粘贴功能，可轻松在相似类型的设置之间移动数值。",
        "新增一个用于管理字体选项的高级设置模板（未来更新将加入更多字体自定义选项）。",
        "#C_更改:_#",
        "设置中的数字滑块外观已更新，以匹配新的暴雪滑块，同时保留所有基于 Widget Tools Toolboxes 的插件增强功能。",
        "Toolbox 的加载结构已全面重构，不再支持旧版本。",
        "许多基础工具函数已移交给 Widget Tools（不再是 Toolbox 专属），可通过 WidgetTools.utilities 全局访问。",
        "Toolbox 专属数据将不再注入到框架表中，而是存放在 Toolbox 专属表中（包括提示信息或容器内容布局数据）。",
        "用于管理暴雪全局 OnEvent（以及自定义事件）处理器的事件处理后台系统已更新，并新增可通过 WidgetTools.utilities 全局访问的工具函数。",
        "可自定义的框架、按钮和其他组件现在必须通过新的构造函数创建，其基础版本中的自定义标记已被移除.",
        "大部分仅用于开发的注释已移出插件安装文件，以大幅减少安装体积。",
        "数据管理设置页面（现称为“配置文件页面”）的构建逻辑已拆分为一个 profilemanager 组件及其上的 GUI 变体，以提供更高的开发灵活性和自定义能力。",
        "其他多项底层改进与优化。",
        "#F_热修复项:_#",
        "大量其他小型修复。",
        "#H_感谢大家的帮助、建议与错误报告 !_# 如果遇到任何问题，请务必反馈！请尽量说明问题发生的时间、方式，以及（如相关）你正在使用的其他插件，以便我能更好地复现并修复问题。如有可能，请附上 Lua 错误信息与 taint 日志。",
    }
}
